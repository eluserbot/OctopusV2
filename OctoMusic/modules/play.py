...
from OctoMusic.config import ARQ_API_KEY
...
async def generate_cover(requested_by, title, views, duration, thumbnail):
    ...
    draw.text((205, 990), f"Title: {title}", (51, 215, 255), font=font)
    draw.text((205, 980), f"Duration: {duration}", (255, 255, 255), font=font)
    draw.text((205, 970), f"Views: {views}", (255, 255, 255), font=font)
    draw.text(
        (205, 960),
        ...
        for administrator in administrators:
            if administrator == message_.from_user.id:
                ...
                try:
                    ...
                    await lel.edit(
                        f"**🔴 Flood Wait Error 🔴 \nUser {user.first_name} couldn't join your group due to heavy requests for userbot! Make sure user is not banned in group."
                        "\n\nOr manually add @VCsMusicPlayer to your Group and try again**",
                    )
    try:
        await USER.get_chat(chid)
        # lmoa = await client.get_chat_member(chid,wew)
